...
cp docs/templates/development/development-core.template docs/development/{topic}.md
...
```bash
# 复制检查指南模板
cp docs/templates/review/review-guide.template docs/development/references/{topic}.md
...
cp docs/templates/development/guideline.template docs/development/{topic}.md
...
```bash
# 复制开发工作流文档模板
cp docs/templates/development/development-workflow.template docs/development/workflows/{workflow-name}.md
...
```bash
# 复制参考文档模板
cp docs/templates/development/development-reference.template docs/development/references/{topic}.md
...
在生成文档时，AI 助手（如 Cursor）可能会在文档末尾添加"最后更新"时间戳，但时间可能不正确（时区、格式等问题）。本指南提供了完整的格式规范、维护流程和最佳实践。
...
2. **格式统一**：使用 `**最后更新**: YYYY-MM-DD` 格式
3. **位置**：放在文档末尾，分隔线 `---` 之后
4. **获取日期**：使用命令行工具或代码函数获取当前日期
