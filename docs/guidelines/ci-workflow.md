...
1. **代码质量检查**：格式化检查、Linter 检查（golangci-lint）、编译检查
2. **测试验证**：运行完整的测试套件
...
**步骤**：
1. Checkout code
2. Setup Go 环境（指定 Go 版本）
3. Cache Go modules
4. `go fmt -d .`：检查代码格式
5. `golangci-lint run`：运行 Linter 检查
6. `go build ./...`：编译检查
...
版本更新分支（`bump-version-*`）只包含版本号的更改（`go.mod` 和 `go.sum`），这些更改：
- 不涉及代码逻辑
- 不需要重新运行 lint 和测试
- 应该快速合并，以便继续发布流程
...
3. 检查 PR 创建者是否是预期的用户
4. 在 GitHub 仓库设置中配置 `WORKFLOW_USER_NAME` Repository Variable
