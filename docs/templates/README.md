...
**重要**：所有模板已更新为 **Go 语言版本**，适用于 Go 项目结构：
- 项目结构：`cmd/workflow/main.go`（入口）、`internal/cli/`（命令定义）、`internal/commands/`（命令实现）、`internal/lib/`（核心业务逻辑）
- 代码示例：使用 Go 语法和标准库
- 测试模板：使用 Go 标准测试框架（`testing` 包）
- 命名规范：遵循 Go 语言命名约定
...
| 模板文件 | 用途 | 包含章节 |
|---------|------|---------|
| `test-case.template` | 测试用例文档 | 测试描述、测试代码、预期结果（Go 语言版本） |
| `mock-test.template` | Mock 测试文档 | 测试描述、Mock 设置、测试代码、注意事项（Go 语言版本） |
| `ignored-test.template` | 忽略测试文档 | 测试描述、忽略原因、恢复计划、临时解决方案（Go 语言版本） |
...
# 例如：创建测试文档
cp docs/templates/testing/test-case.template docs/testing/test-my-feature.md
```
...
**注意**：所有模板已更新为 Go 语言版本，代码示例使用 Go 语法，项目结构遵循 Go 项目规范。
...
- **时间戳**：在文档末尾添加 "最后更新" 时间戳（参考 `docs/guidelines/document.md` 中的"文档时间戳维护"章节）
- **索引更新**：如有需要，更新相应的文档索引（`docs/README.md` 或 `docs/requirements/README.md`）
- **代码规范**：代码示例遵循 Go 语言规范和项目命名约定（包名小写，公开函数/变量 PascalCase，私有函数/变量 camelCase）
...
- [文档编写指南](../guidelines/document.md) - 文档编写规范和"最后更新"时间戳格式要求（包含文档时间戳维护章节）
- [开发指南索引](../development/README.md) - 开发规范和最佳实践
- [测试指南索引](../testing/README.md) - 测试规范和测试工具
