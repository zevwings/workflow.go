...
reportPath := fmt.Sprintf("report/REVIEW_CLI_%s.md", timestamp)
```
...
timestamp := FormatFilenameTimestamp()
reportPath := fmt.Sprintf("report/REVIEW_CODE_%s.md", timestamp)
```
...
# 检查单元测试
grep -r "func Test" . -name "*_test.go"
```
...
timestamp := FormatFilenameTimestamp()
reportPath := fmt.Sprintf("report/REVIEW_TEST_%s.md", timestamp)
```
...
timestamp := FormatFilenameTimestamp()
reportPath := fmt.Sprintf("report/REVIEW_DOCUMENT_%s.md", timestamp)
```
...
# 检查模块结构
MODULE=pr
find internal/lib/$MODULE -name "*.go" -type f | grep -v "_test.go" | sort

# 统计代码行数
find internal/lib/$MODULE -name "*.go" -type f | grep -v "_test.go" | xargs wc -l | tail -1

# 提取公共 API
grep -r "func [A-Z]\|type [A-Z]\|const [A-Z]" internal/lib/$MODULE/ | head -20
```
...
timestamp := FormatFilenameTimestamp()
reportPath := fmt.Sprintf("report/REVIEW_ARCHITECTURE_DOC_%s.md", timestamp)
```
...
timestamp := FormatFilenameTimestamp()
reportPath := fmt.Sprintf("report/review-report-%s.md", timestamp)
