...
- [ ] 所有单元测试通过
- [ ] 所有集成测试通过
- [ ] 补全脚本完整性测试通过（`go test ./internal/cli/...`）
...
```bash
# 参考架构文档审查指南
# docs/development/references/review-architecture-consistency.md

# 快速检查命令示例：
# 1. 检查模块结构
MODULE=pr
find internal/lib/$MODULE -name "*.go" -type f | sort

# 2. 统计代码行数
find internal/lib/$MODULE -name "*.go" -type f | xargs wc -l | tail -1
