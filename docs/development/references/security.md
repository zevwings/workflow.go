...
**参考**：`internal/commands/config.go` 中的 `FilterSecrets` 实现。
...
input, err := prompt.Input("Enter JIRA ID", prompt.WithValidator(func(s string) error {
    if s == "" {
        return fmt.Errorf("JIRA ID cannot be empty")
    }
    if !strings.Contains(s, "-") {
        return fmt.Errorf("Invalid JIRA ID format (expected: PROJ-123)")
    }
    return nil
}))
if err != nil {
...
```go
// ❌ 不安全：直接拼接用户输入
cmd := fmt.Sprintf("git checkout %s", userInput)
exec.Command("sh", "-c", cmd).Run()

// ✅ 安全：使用参数化命令
import "os/exec"
cmd := exec.Command("git", "checkout", userInput)
cmd.Run()
```
...
// ✅ 安全：规范化路径并检查
path, err := filepath.Abs(userInput)
if err != nil {
...
**规则**：定期使用 `govulncheck` 或 `nancy` 检查依赖安全漏洞。
...
**检查步骤**：
1. 使用 `govulncheck` 或 `nancy` 检查依赖是否有已知漏洞
2. 检查依赖的维护状态（是否活跃维护）
3. 检查依赖的许可证是否兼容
4. 评估依赖的安全记录
...
1. 使用 `MaskSensitiveValue()` 函数过滤敏感信息（如适用）
2. 检查所有日志输出点
3. 检查配置导出功能
...
- `internal/lib/util/string.go`（如适用） - 敏感信息处理工具（`MaskSensitiveValue` 函数）
- `internal/commands/config/export.go`（如适用） - 配置导出时的敏感信息过滤（`FilterSecrets` 函数）
- `internal/lib/prompt/input.go` - 输入验证机制（`Input` 验证器）
