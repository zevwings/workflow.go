...
# 检查命令层模块
COMMAND=pr
find internal/commands/$COMMAND -name "*.go" -type f | grep -v "_test.go" | sort
```
...
# 统计模块文件数量
MODULE=pr
find internal/lib/$MODULE -name "*.go" -type f | grep -v "_test.go" | wc -l
```
...
```bash
# 提取公共函数和结构体（Go 语言中，首字母大写的为导出函数/类型）
MODULE=pr
grep -r "^func [A-Z]\|^type [A-Z]\|^const [A-Z]\|^var [A-Z]" internal/lib/$MODULE/ | head -20
```
...
- [ ] **外部依赖一致性**：文档中列出的外部依赖是否与实际 go.mod 一致
...
```bash
# 查找错误类型定义（Go 语言中，错误通常是 error 接口或自定义错误类型）
grep -r "type.*Error\|func.*error\|errors\." internal/lib/$MODULE/
```
