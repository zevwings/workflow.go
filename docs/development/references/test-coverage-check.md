...
```bash
# 查看新增的公共 API（Go 语言中，首字母大写的为导出函数/类型）
git diff main...HEAD | grep -E "^\+.*func [A-Z]|^\+.*type [A-Z]|^\+.*const [A-Z]"
...
```bash
# 查找新增功能的测试文件
find internal -name "*_test.go" -exec grep -l "新增功能名称" {} \;

# 检查测试文件是否存在
find internal -name "*_test.go" | grep "新增模块名"

# 运行特定模块的测试
go test ./internal/lib/模块名
```
...
```bash
# 查找对应的集成测试文件
find . -name "*_test.go" -path "*/test*" -exec grep -l "新增功能名称" {} \;

# 检查测试文件是否存在
ls test/新增模块名/*_test.go
...
```bash
# 列出所有新增的公共函数
git diff main...HEAD | grep -E "^\+.*func [A-Z]" | sed 's/^+//' | awk '{print $2}'

# 检查每个函数是否有测试
for func in $(git diff main...HEAD | grep -E "^\+.*func [A-Z]" | sed 's/^+//' | awk '{print $2}'); do
    echo "检查函数: $func"
    grep -r "Test.*$func\|$func.*test" . -name "*_test.go" || echo "  ⚠️  未找到测试"
done
```
...
echo "## 新增公共函数测试覆盖"
echo ""
git diff main...HEAD | grep -E "^\+.*func [A-Z]" | while read line; do
