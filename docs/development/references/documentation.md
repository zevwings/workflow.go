...
所有公共函数、结构体、类型必须添加文档注释：

```go
// DownloadFromJira 下载指定 JIRA ticket 的日志文件
//
// 参数:
//   - ticketID: JIRA ticket ID（如 "PROJ-123"）
//
// 返回:
//   - []byte: 下载的日志文件字节数据
//   - error: 如果下载失败，返回错误信息
...
//
//	logs := NewJiraLogs()
//	data, err := logs.DownloadFromJira("PROJ-123")
//	if err != nil {
//		return err
//	}
func (j *JiraLogs) DownloadFromJira(ticketID string) ([]byte, error) {
	// 实现
}
```
...
1. **代码示例应该可以编译和运行**：使用 `go test` 验证
2. **代码示例应该包含必要的导入**：确保所有使用的类型都已导入
3. **代码示例应该包含断言验证结果**：使用 `if` 语句或其他断言方法
4. **避免忽略错误**：应该检查错误并处理，不要使用 `_` 忽略错误
5. **使用正确的包路径**：确保导入路径正确（使用相对路径或完整路径）

#### 示例：好的做法

```go
// FormatFilenameTimestamp 生成文件名时间戳（格式：YYYY-MM-DD_HH-MM-SS）
//
// 示例:
//
...
```go
// FormatFilenameTimestamp 生成文件名时间戳
//
// 示例:
//
//	timestamp := FormatFilenameTimestamp()  // ❌ 缺少导入
//	fmt.Println(timestamp)  // ❌ 没有断言验证
func FormatFilenameTimestamp() string {
	// 实现
}
```
...
使用以下命令运行测试（Go 没有单独的文档测试，测试包含在单元测试中）：
