...
**注意**：Workflow CLI 支持通过模板系统自定义分支命名格式，可在 `.workflow/config.toml` 或全局配置文件中配置 `[template.branch]` 部分。
...
分支命名模板可在 `.workflow/config.toml` 或全局配置文件中通过 `[template.branch]` 部分进行配置。
