...
本目录用于存放 Workflow CLI 项目的开发规范文档，定义代码风格、错误处理、命名约定、模块组织、开发流程等各个环节的规范和最佳实践。

## 📋 说明

开发规范文档用于指导开发者编写高质量、可维护的代码，确保代码风格统一、错误处理规范、开发流程清晰。
...
**开发规范模板位置**：`docs/templates/development/`

该目录提供了以下模板：

- **核心规范模板**（`development-core.template`）：核心开发规范的模板
- **参考文档模板**（`development-reference.template`）：参考文档的模板
- **工作流模板**（`development-workflow.template`）：开发工作流的模板
- **指南模板**（`guideline.template`）：指南类文档的模板

**使用场景**：
- 创建新的开发规范文档时，参考对应的模板结构
- 确保文档格式和内容结构符合规范
- 保持文档风格一致
...
- 核心规范文档存放在 `docs/development/` 目录下
- 参考文档存放在 `docs/development/references/` 目录下
- 工作流文档存放在 `docs/development/workflows/` 目录下
...
- 错误类型（统一使用 `error` 接口）
- 错误信息格式规范
- 错误处理模式（`fmt.Errorf`、`errors.Wrap`、`errors.Is` 等）
- 分层错误处理
...
**关键内容**：
- 文件命名（Go 源文件、测试文件、文档文件）
- 函数命名（动作函数、查询函数、检查函数）
- 结构体命名
- 常量命名（驼峰命名或 `SCREAMING_SNAKE_CASE`）
- CLI 参数命名规范
...
**关键内容**：
- 审查清单（代码格式化、golangci-lint 检查、测试通过、文档更新等）
...
- [核心规范模板](../templates/development/development-core.template)
- [参考文档模板](../templates/development/development-reference.template)
- [工作流模板](../templates/development/development-workflow.template)
- [指南模板](../templates/development/guideline.template)

### 架构文档

- [架构文档总览](../architecture/README.md) - 项目架构总览
...
1. 根据文档类型选择合适的模板：
   - 核心规范 → 使用 `development-core.template`
   - 参考文档 → 使用 `development-reference.template`
   - 工作流文档 → 使用 `development-workflow.template`
   - 指南文档 → 使用 `guideline.template`

2. 复制模板文件到对应目录：
   ```bash
   cp docs/templates/development/{模板名}.template docs/development/{文档名}.md
   ```
...
所有开发规范模板位于 `docs/templates/development/` 目录下。
