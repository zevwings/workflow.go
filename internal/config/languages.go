...
//   - languageCode: 语言代码（如果为空，使用默认值 "en"）
//
// 返回:
...
// 说明:
//
//	语言选择优先级：传入的语言代码 > 默认值（"en"）
//	如果语言代码不在支持列表中，将使用英文作为默认语言。
func GetLanguageRequirement(systemPrompt string, languageCode string) string {
	// 如果语言代码为空，使用默认值
	if languageCode == "" {
		languageCode = "en"
	}
